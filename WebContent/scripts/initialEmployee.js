let sessionId;
const isUserUrl = "http://localhost:8081/reimbursmentsystem/session";

function validateUser(xhr) {
  let response = JSON.parse(xhr.responseText);
  if (response == null || response.email == null) {
    window.location = "http://localhost:8081/reimbursmentsystem/login";
  }
  ...
  let initUrl = `${reimbursmentUrl1}?employeeId=${sessionId}&currentState=pending`;
...
const reimbursmentUrl1 = "http://localhost:8081/reimbursmentsystem/reimbursment";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  submit.addEventListener("click", function (e) {
    ...
    if ((amount.value < 0 || amount.value == "") || reason.value.length < 6) {
      ...
      amount.value = "";
      reason.value = "";
      sendAjaxPost(reimbursmentUrl1, function () {
        showAlert();
  ...
  function populatePending() {
    ...
    tbody.innerHTML = "";
    let url = `${reimbursmentUrl1}?employeeId=${sessionId}&currentState=pending`;
  ...
  function populateResolved() {
    ...
    let url = `${reimbursmentUrl1}?employeeId=${sessionId}&currentState=resolved
    `;
    sendAjaxGet(url, populateTable);
  }
